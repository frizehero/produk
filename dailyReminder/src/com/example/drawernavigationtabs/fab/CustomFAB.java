package com.example.drawernavigationtabs.fab;
...
public class CustomFAB extends ImageButton {
	...
	private void init(AttributeSet attrSet) {
		...
		TypedArray arr = theme.obtainStyledAttributes(attrSet, R.styleable.FAB,
				0, 0);
		try {
			setBgColor(arr.getColor(R.styleable.FAB_bg_color,
					Color.rgb(97, 166, 167)));
			setBgColorPressed(arr.getColor(R.styleable.FAB_bg_color_pressed,
					Color.GRAY));
			StateListDrawable sld = new StateListDrawable();

			sld.addState(new int[] { android.R.attr.state_pressed },
					createButton(bgColorPressed));
